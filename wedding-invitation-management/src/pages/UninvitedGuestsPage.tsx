...
import { useMarkInvitationSent } from '../hooks/guest/useMarkInvitationSent';

export default function UninvitedGuestsPage() {
  const navigate = useNavigate();
  const { logout } = useAuth();
  const { guests, count, isLoading, error, fetchGuests } = useGetGuestsByInvitationStatus();
  const { isLoading: markingInvitation, markInvitationSent } = useMarkInvitationSent();
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');

  useEffect(() => {
    fetchGuests('uninvited');
  }, [fetchGuests]);
  ...
  const handleMarkAsSent = async (guestId: string) => {
    const result = await markInvitationSent(guestId);
    if (result.success) {
      toast.success('Invitation marked as sent');
      fetchGuests('uninvited');
    } else {
      toast.error('Failed to mark invitation as sent');
    }
  };
          ...
          <div className="flex justify-between items-center h-14 sm:h-16">
            ...
            <div className="flex items-center">
              <button
                ...
                className="mr-2 p-2 rounded-lg text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-amber-500"
              >
            ...
            <button
              ...
              className="md:hidden p-2 rounded-lg text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-amber-500"
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-6 sm:py-8 lg:py-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Uninvited Invitations</h1>
              <p className="text-sm text-gray-500">{count} invitation{count !== 1 ? 's' : ''} not yet sent</p>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="block w-full sm:w-64 pl-10 pr-4 py-2 border border-gray-300 rounded-lg bg-white/50 text-gray-800 placeholder-gray-400 focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all duration-200"
            />
          </div>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg border border-white/50">
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-2">No uninvited invitations</p>
              <p className="text-sm text-gray-500">All invitations have been sent</p>
            </div>
            ...
            <>
                ...
                {filteredGuests.map((guest) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <span className="inline-flex items-center justify-center px-2 py-1 bg-amber-200 text-amber-700 text-xs font-medium rounded-full">
                        {guest.confirmedGuests}/{guest.numberOfGuests}
                      </span>
                    </div>
                    {guest.categoryName && (
                      <p className="text-xs text-gray-500 mb-1">Category: {guest.categoryName}</p>
                    )}
                    {guest.tableName && (
                      <p className="text-xs text-gray-500 mb-2">Table: {guest.tableName}</p>
                    )}
                    <button
                      onClick={() => guest._id && handleMarkAsSent(guest._id)}
                      disabled={markingInvitation}
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {filteredGuests.map((guest) => (
                      <tr key={guest._id} className="border-b border-gray-100 hover:bg-amber-50/50">
                        ...
                        <td className="py-3 px-4 text-center">
                          <span className="inline-flex items-center justify-center px-2 py-1 bg-amber-100 text-amber-700 rounded-full text-sm font-medium">
                            {guest.confirmedGuests}/{guest.numberOfGuests}
                          </span>
                        </td>
                        <td className="py-3 px-4 text-center">
