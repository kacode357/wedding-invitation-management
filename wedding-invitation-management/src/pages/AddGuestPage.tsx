...
export default function AddGuestPage() {
  ...
  const [guests, setGuests] = useState<GuestDraft[]>(() => {
    ...
    return [{ id: Date.now().toString(), guestName: '', categoryId: '', numberOfGuests: 1, confirmedGuests: 1, noteId: '' }];
  });

  const [openDropdown, setOpenDropdown] = useState<{ index: number; type: 'category' | 'guest' | 'confirmed' | 'note' } | null>(null);
  ...
  const toggleDropdown = (index: number, type: 'category' | 'guest' | 'confirmed' | 'note', e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (success) {
      ...
      setGuests([{ id: Date.now().toString(), guestName: '', categoryId: '', numberOfGuests: 1, confirmedGuests: 1, noteId: '' }]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-amber-50 to-rose-100 pb-20" onClick={() => setOpenDropdown(null)}>
      ...
      <main className="max-w-3xl mx-auto px-3 sm:px-4 lg:px-8 py-6 sm:py-8">
        ...
        {(isLoadingCategories || isLoadingNotes) && (
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <button type="button" onClick={(e) => !isLoadingCategories && toggleDropdown(index, 'category', e)} disabled={isLoadingCategories}
                        className={`w-full px-4 py-3 text-left border rounded-xl bg-white transition-all duration-200 flex items-center justify-between ${isLoadingCategories ? "bg-gray-100 cursor-not-allowed border-gray-200" : guest.categoryId !== '' ? "border-rose-400" : "border-gray-300 hover:border-rose-400"}`}>
                        <span className={guest.categoryId === '' ? 'text-gray-400' : 'text-gray-800'}>{selectedCategory?.name || 'Select a category'}</span>
                        <svg className={`w-5 h-5 text-gray-400 transition-transform ${openDropdown?.index === index && openDropdown.type === 'category' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg>
                      </button>
          ...
          <div className="space-y-4">
            ...
            <button type="submit" disabled={isLoadingCategories || isLoadingNotes || isLoadingGuests}
