...
export default function CategoriesPage() {
  ...
  const { deleteCategory, isLoading: deleteLoading, error: deleteCategoryError } = useDeleteCategory();
  ...
  const [editFormData, setEditFormData] = useState<UpdateCategoryPayload>({});
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    if (!categoryId) {
      setEditError('Invalid category ID');
      return;
    }

    setEditError(null);
    const result = await updateCategory(categoryId, editFormData);
    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-amber-50 to-rose-100">
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-6 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 sm:mb-8">
          <div>
            <h1 className="text-2xl sm:text-3xl font-serif font-bold text-gray-800">Categories</h1>
            <p className="text-gray-600 mt-1">Total: {categories.length} categories</p>
          </div>
          <button
            onClick={() => navigate('/add-category')}
            className="flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-500 text-white rounded-xl hover:bg-indigo-600 transition-colors font-medium shadow-md"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="mb-6">
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-3 pl-12 bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm"
            />
            <svg
            ...
            <form onSubmit={handleEditSubmit} className="p-4 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-gray-800"
                  required
                />
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 px-4 bg-indigo-500 text-white font-semibold rounded-xl hover:bg-indigo-600 transition-colors disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {updateLoading ? (
                  <>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            {/* Title and Message */}
            <h3 className="text-lg font-semibold text-gray-800 text-center mb-2">Delete Category</h3>
            ...
            {(deleteError || deleteCategoryError) && (
              <div className="mb-4 bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-xl flex items-center gap-2">
                ...
                <span>{deleteError || deleteCategoryError}</span>
              </div>
            )}
